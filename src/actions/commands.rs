...
impl App {
    pub fn next_command(&mut self) {
        ...
        let commands_len = self.commands.get_current_list_len();
        if commands_len > 0 {
            self.commands.selected_command = (self.commands.selected_command + 1) % commands_len;
        }
    }
    ...
    pub fn prev_command(&mut self) {
        ...
        if self.commands.selected_command == 0 {
            return self.commands.selected_command = commands_len - 1;
        }
